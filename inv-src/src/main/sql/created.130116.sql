...
CREATE  TABLE IF NOT EXISTS `nodes` (
  ...
  `number` INT NOT NULL ,
  `media_type` ENUM('magnetic-disk','magnetic-tape','optical-disk','solid-state', 'unknown') NOT NULL DEFAULT 'unknown' ,
  `media_connectivity` ENUM('cloud','das','nas','san', 'unknown') NOT NULL DEFAULT 'unknown' ,
  `access_mode` ENUM('on-line', 'near-line', 'off-line', 'unknown') NOT NULL DEFAULT 'unknown' ,
  `access_protocol` ENUM('cifs','nfs','open-stack','s3','zfs', 'unknown') NOT NULL DEFAULT 'unknown' ,
...
-- Table `nodes_objects`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nodes_objects` ;

CREATE  TABLE IF NOT EXISTS `nodes_objects` (
  `id` INT NOT NULL ,
  `nodes_id` SMALLINT UNSIGNED NOT NULL ,
  `objects_id` INT UNSIGNED NOT NULL ,
  `role` ENUM('primary','secondary') NOT NULL ,
  `storage_url` VARCHAR(2046) NOT NULL ,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  `modified` TIMESTAMP NULL ,
  PRIMARY KEY (`id`) ,
  KEY `id_idx` (`nodes_id` ASC) ,
  KEY `id_idx1` (`objects_id` ASC) ,
-- CONSTRAINT `id`
