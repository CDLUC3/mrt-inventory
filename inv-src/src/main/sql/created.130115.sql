...
-- Modify the output of MySQL Workbench chaning "INDEX" to "KEY" and
...
CREATE  TABLE IF NOT EXISTS `nodes` (
  ...
  `number` INT NOT NULL ,
  `media_type` ENUM('magnetic-disk','magnetic-tape','optical-disk','solid-state') NOT NULL ,
  `access_mode` ENUM('on-line', 'near-line', 'off-line') NOT NULL ,
  `connectivity` ENUM('cloud','das','nas','san') NOT NULL ,
  `protocol` ENUM('cifs','nfs','open-stack','s3','zfs') NOT NULL ,
  `verify_on_read` TINYINT(1) NOT NULL ,
  `verify_on_write` TINYINT(1) NOT NULL ,
  `base_url` VARCHAR(2046) NOT NULL ,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  `modified` TIMESTAMP NULL ,
  PRIMARY KEY (`id`) )
ENGINE = InnoDB;
