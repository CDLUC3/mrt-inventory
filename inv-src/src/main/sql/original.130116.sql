...
CREATE  TABLE IF NOT EXISTS `nodes` (
  ...
  `media_connectivity` ENUM('cloud','das','nas','san') NOT NULL ,
  `access_mode` ENUM('on-line', 'near-line', 'off-line') NOT NULL ,
  `access_protocol` ENUM('cifs','nfs','open-stack','s3','zfs') NOT NULL ,
...
CREATE  TABLE IF NOT EXISTS `nodes_objects` (
  ...
  `created` TIMESTAMP NOT NULL DEFAULT DEFAULT_TIMESTAMP ,
  `modified` TIMESTAMP NULL ,
  PRIMARY KEY (`id`) ,
