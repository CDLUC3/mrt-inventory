...
CREATE  TABLE IF NOT EXISTS `inv_nodes` (
  ...
  `node_form` ENUM('physical', 'virtual') NOT NULL DEFAULT 'virtual',
  ...
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  `source_node` SMALLINT UNSIGNED NULL ,
  `target_node` SMALLINT UNSIGNED NULL ,
  PRIMARY KEY (`id`) )
ENGINE = InnoDB;
...
CREATE  TABLE IF NOT EXISTS `inv_audits` (
  ...
  `modified` TIMESTAMP NULL,
  `failed_size` BIGINT UNSIGNED NOT NULL DEFAULT 0 ,
  `failed_digest_value` VARCHAR(255) NULL ,  
   `note` text NULL,
  PRIMARY KEY (`id`) ,
  KEY `id_idx` (`inv_node_id` ASC) ,
  KEY `id_idx1` (`inv_object_id` ASC) ,
