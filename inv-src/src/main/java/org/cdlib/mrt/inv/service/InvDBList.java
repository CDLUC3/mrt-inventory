        ...
        throws TException
    {
        try {
            ...
            for (int iOut=0; true; iOut++) {
                ...
                int nodeID = -1;
                if (parts.length < 2) {
                    logger.logError("Badly formatted line:" + line, 0);
                    continue;
                }
                try {
                    ...
                    logger.logError("Badly formatted node - must be numeric:" + parts[0], 0);
                    continue;
                }
                
                String arkS = parts[1];
                if (!arkS.startsWith("ark")) {
                    System.out.println("entry invalid - not ark:" + arkS);
                    continue;
                }
                Identifier objectID = new Identifier(arkS);
                for (int itry=0; itry < 3; itry++) {
                    try {
                        service.add(nodeID, objectID);
                        break;
