...
public class VersionsTest
{
    ...
    public static void main(String args[])
    {
        ...
        try {
            ...
            Properties prop = InvDBUtil.getVersionsStuff(objectID, connect, logger);
            System.out.println(PropertiesUtil.dumpProperties("test1", prop));
            
            for (long v=1; true; v++) {
                List<InvFile> files = InvDBUtil.getFiles(objectID, v, connect, logger);
                if (files == null) break;
                System.out.println("***Version:" + v + "***");
                for (InvFile file : files) {
                    Properties fileProp = file.retrieveProp();
                    System.out.println(PropertiesUtil.dumpProperties("-File-", fileProp));
                }
            }
            /*
            if (nodes == null) {
